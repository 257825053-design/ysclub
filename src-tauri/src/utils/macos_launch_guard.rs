...
mod tests {
    ...
    fn canonical_allowed_roots_and_nested_bundles_are_accepted() -> anyhow::Result<()> {
        ...
        let system_exe = executable(&system.join("Tools/YSCLUB.app"))?;
        let user_exe = executable(&user.join("Network/YSCLUB.app"))?;
    ...
    fn failed_staged_activation_preserves_existing_application() -> anyhow::Result<()> {
        ...
        let destination = root.join("YSCLUB.app");
        let staging = root.join(".YSCLUB.app.installing");
        let backup = root.join(".YSCLUB.app.backup");
    ...
    fn canonicalization_accepts_symlink_in_and_rejects_symlink_escape() -> anyhow::Result<()> {
        ...
        let allowed_bundle = system.join("YSCLUB.app");
        let allowed_exe = executable(&allowed_bundle)?;
        let link_in = downloads.join("YSCLUB.app");
    ...
    fn allowed_root_symlink_escape_is_not_authorized() -> anyhow::Result<()> {
        ...
        let escaped_exe = executable(&home.join("Applications/YSCLUB.app"))?;
    ...
    fn translocation_and_missing_bundle_are_rejected_before_side_effects() {
        let translocated =
            std::path::Path::new("/private/var/folders/AppTranslocation/YSCLUB.app/Contents/MacOS/clash-verge");
        assert_eq!(
